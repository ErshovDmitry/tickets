...
Положите бинарник в `PATH` (`~/bin/ticket`; `%USERPROFILE%\bin\ticket.exe` на Windows). Он сам находит каталог тикетов из любой текущей директории. Порядок резолюции (первое совпадение выигрывает):
...
> **Migration note.** Проекты, созданные старыми версиями, хранят бинарь или симлинк в `tickets/bin/` — при вызове изнутри дерева проекта они продолжают работать (восходящий поиск). Что больше не работает: вызов этого бинаря по абсолютному пути из посторонней директории в расчёте на его расположение. Используйте `-C <каталог-тикетов>` или `TICKETS_DIR`. Устаревшую копию `tickets/bin/` можно удалить в удобный момент.
...
Это создаст `tickets/` и `tickets/archive/`. Идемпотентно. Бинарь НЕ устанавливает — держите один `ticket` в `PATH`.

### Один бинарь на все проекты
...
Флаг `-C`/`--tickets-dir` (см. [Установку](#установка)) перекрывает `TICKETS_DIR` на один вызов.
...
Локальная сборка — см. [Сборка из исходников](#сборка-из-исходников). Для Windows: кросс-компиляция `dist/ticket.exe` (windows/amd64, таблица выше), затем прогон `scripts/smoke-windows.ps1` на Windows-хосте (вручную по SSH). Smoke проверяет `new`/`list`/`show`/`set`, глобальный флаг `-C`/`--tickets-dir` из чужой рабочей директории, override `TICKETS_DIR` и параллельный `new` ×5 (пять уникальных последовательных номеров через OS-лок) — целиком внутри временной песочницы, репозиторий и данные не затрагиваются.
